...
const ComponentEvent: React.FC = () => {
    const { curComponent, updateComponent, components,getComponentById } = useLowCodeStore();
    const { componentConfig } = useLowCodeConfigStore();
    const [actionModalOpen, setActionModalOpen] = useState(false);
    const [curEvent, setCurEvent] = useState<LowCodeComponentEvent>();
    const [curAction, setCurAction] = useState<ActionType>();
    const [curActionIndex, setCurActionIndex] = useState<number>();

    if (!curComponent) return null

    function deleteAction(event: LowCodeComponentEvent, index: number) {
